...
- TB2 主線不再追求成為 Codex 原生 remote-control 的對等替代品
- 平台策略接受並文件化 Windows / WSL 雙軌：native Windows 偏向日常低摩擦使用，WSL 偏向高穩定 terminal 協作
- 保留一條獨立的外部 runtime / workflow 實驗線，作為這條雙軌策略下的驗證沙盒
- `codex_bridge_service` 因完全服務於 Codex 原生遙控能力，現決議直接關閉，不列入 remediation 或主線功能擴充

目前 branch 已完成第一個短期 milestone，並已開始吃掉後續 backlog：
...
- `P1` 第 9 項的 machine-readable source 欄位已在 room payload 與 bridge event 中落地
- `P2` 第 10 項的 auto-forward rate limit / circuit breaker 已在 bridge guard 與對應測試中落地
- 本地完整驗證為 `.venv/bin/python -m pytest -q` => `310 passed in 14.59s`

這裡是 `2026-03-28` 的歷史快照，不代表目前最新主線驗證結果。

目前剩餘的高價值項目主要集中在：

- 更完整的 environment diagnostics 與 support matrix 對齊
- 持久化 audit trail 的設計與範圍定義
- GUI / docs 對 source 與 guard 狀態的操作說明
- 治理層級與 policy provenance 的單一解析契約
...
- 文件敘述要和實際安全邊界一致
- 驗收條件必須可測試，不只靠主觀描述
- 治理 policy 要有單一 precedence 與 provenance，不再散落在 README、preset 與 operator 默契

## 治理分層參考

治理層目前採用 repo 內文件化的漏斗狀層疊契約：
...
也就是先能解析並報告「目前應生效的 governance 是什麼」，再決定哪些 key 可以安全接入 runtime apply / rollback。
這裡的 no-mutation 是指 future layered resolver，而不是否定目前已存在的 per-workstream mutable controls。
...
這不是當前必做的 blocking fix，但如果產品方向是 operator control plane / governance layer，遲早要補。
