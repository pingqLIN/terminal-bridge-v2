...
- 本地 branch 目前只有 `main`
- worktree 不是乾淨狀態，存在未追蹤 `.codex`
- remote 仍有其他分支：
  - `origin/copilot/fix-human-intervention`
  - `origin/copilot/update-documentation-for-project`
  - `origin/copilot/update-readme-with-image`
  - 3 條 `dependabot/*`
...
- 在 worktree 尚有未追蹤 `.codex` 的情況下，不適合直接做任何 branch deletion、history rewrite、或 commit 壓縮操作

後續治理補充：

- 已將 repo 根目錄 `.codex` 加入 ignore，避免持續污染 worktree
- 已確認 `origin/copilot/fix-human-intervention` 完全 merged，可安全刪除
- 其餘 remote branches 仍帶有獨立 commit，暫不自動刪除
...
- sandbox 外重新執行 `pytest -q`：
  - `345 passed in 13.24s`

結論：

- 目前主線在真實本機環境下可通過完整測試
- 測試數量已從舊文件中的 `285`、`310` 進一步成長到 `345`
...
- 但 runtime `status()` 仍然是 `rooms + bridges + bridge_details` 為中心
- 真正的 `workstream_id`、fleet-safe mutation targeting、per-workstream policy 還沒有成為 server 主資料模型

判讀：

- `1 + n` 已經進入 UI 與架構規劃期
- 但目前仍屬於「bridge/room 雛形可承載多條線」，不是正式完成的 fleet architecture
...
3. 已確認 merged 的 remote branch 應清除，未 merged 的 remote branches 不應直接硬刪
4. 不適合在現在直接做「重寫 commit 歷史」

若你要把 repo 層級也整理成「只剩 main」，建議先做一個單獨治理批次：

- 先確認 `.codex` 是否應加入 `.gitignore` 或移出 repo
- 盤點 `copilot/*` 與 `dependabot/*` 是否已被合併或可刪
- 由 repo owner 執行 remote branch policy 整理
...
- 本地 branch 已只剩 `main`
- 完整測試在真實環境下 `345 passed`
- 專案核心能力已進入「可用且有回歸保護」階段

### 本輪已確認風險

- worktree 尚有 `.codex` 未追蹤
- remote 非主線 branch 仍存在
- `1 + n` 尚未正式完成 runtime model
...
`Phase 0 + Phase 1 的第一切片`

也就是：

- 先把 repo 清到乾淨
- 再定義 `workstream_id` 與新的 `status()` 聚合契約

原因很直接：

- 這能同時解決 repo 治理與產品架構兩條主線的阻力
- 也能讓後續 GUI / MCP / recovery 的工作有穩定錨點
