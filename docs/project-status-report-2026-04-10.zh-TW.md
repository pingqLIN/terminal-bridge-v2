...
- `1 + n` 雖已有正式 runtime model 與 enforcement contract，但 policy inheritance 與 deeper recovery automation 尚未完成
...
`Phase 10` 的 recovery hardening / GUI modularization 切片

也就是：

- 把 best-effort restore 推進到更完整的 recovery-hardening
- 把 GUI 從單檔大模板繼續拆成可維護模組
- 再決定 parent / child policy inheritance 與更高階 orchestration 規則

原因很直接：

- enforcement contract 已經存在，下一輪應該把治理從「單條 workstream 能控」推進到「整個 fleet 更可恢復、更可維護」
- 也能把未來的 GUI 工作聚焦在結構化拆分，而不是再疊更多 inline 行為
