...
它現在也會回傳 machine-readable `runtime` contract，讓 client 區分哪些 state 是 direct-run `memory_only`、哪些是 service-managed `best_effort_restore`，以及哪些 live collaboration pieces 在 service restart 後仍需要 operator recovery。請把 `launch_mode`、`snapshot_schema_version`、`audit_policy_persistence`、`restart_behavior`、`continuity.mode` 視為正式欄位，用來區分 direct local run、service-managed fresh start、restart 後 state lost，或 best-effort restored workstream snapshots。當 TB2 走 managed service path 時，`audit_policy_persistence=service_state` 代表 restart 會延續 audit policy 輸入；restored workstream snapshots 是 handoff aid，不代表 live terminal processes、room subscriptions、bridge processes 或 pending interventions 也存活。
