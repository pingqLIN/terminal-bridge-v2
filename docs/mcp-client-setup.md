...
It now also returns a machine-readable `runtime` contract so the client can see which state is direct-run `memory_only`, which state is service-managed `best_effort_restore`, and which live collaboration pieces still require operator recovery after service restart. Treat `launch_mode`, `snapshot_schema_version`, `audit_policy_persistence`, `restart_behavior`, and `continuity.mode` as the stable fields for distinguishing direct local runs, service-managed fresh starts, restart-after-loss flows, and best-effort restored workstream snapshots. When TB2 is running under the managed service path, `audit_policy_persistence=service_state` means a restart carries forward audit policy inputs; restored workstream snapshots are a handoff aid, not proof that live terminal processes, room subscriptions, bridge processes, or pending interventions survived.
